...
const About = () => {
  ...
  return (
    <section id="about" className="py-20 bg-dark-2 text-light relative overflow-hidden">
        ...
        <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-20">
          ...
          >
            <h3 className="text-2xl md:text-3xl font-bold text-white mb-6">
              Desenvolvedor Front-End com foco em <span className="text-primary">Performance</span> e <span className="text-primary">Design</span>.
            </h3>
            
            <p className="text-light-muted text-lg leading-relaxed mb-8">
              Sou um desenvolvedor apaixonado por criar soluções web modernas e escaláveis. 
              Com sólida experiência em React, Next.js e TypeScript, meu objetivo é entregar 
              não apenas código limpo, mas experiências digitais que impactam usuários. 
              Sempre em busca de novas tecnologias para aprimorar minhas habilidades e entregar resultados de excelência.
            </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-10">
              {info.map((item, index) => (
                <div key={index} className="flex items-center gap-2">
                    ...
                    <span className="text-white font-medium min-w-[100px]">{item.label}:</span>
                    <span className="text-light-muted">{item.value}</span>
                </div>
              ))}
            </div>
            ...
            <div className="flex flex-wrap gap-4">
              <a 
                href="/files/cv.pdf"
                download
                className="px-8 py-3 bg-primary text-white font-medium rounded-full hover:bg-primary-dark transition-all duration-300 flex items-center gap-2 shadow-lg shadow-primary/25"
              >
